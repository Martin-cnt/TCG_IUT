...
function renderStats() { const count = ownedCount(); const percent = Math.round((count / cardNames.length) * 100); const values = [['#heading-progress', count], ['#binder-progress-number', `${percent}%`], ['#binder-progress-label', `${count} / ${cardNames.length} cartes`], ['#all-count', cardNames.length], ['#found-count', count], ['#missing-count', cardNames.length - count], ['#pack-count-label', `${state.packs} pack${state.packs > 1 ? 's' : ''} ouvert${state.packs > 1 ? 's' : ''}`]]; values.forEach(([selector, value]) => { const element = $(selector); if (element) element.textContent = value; }); const progressBar = $('#binder-progress-bar'); if (progressBar) progressBar.style.width = `${percent}%`; }
function renderBinder(filter = 'all') { const binderGrid = $('#binder-grid'); if (!binderGrid) return; const indices = [...Array(cardNames.length).keys()].filter((index) => filter === 'all' || (filter === 'found' ? state.owned[index] : !state.owned[index])); binderGrid.innerHTML = indices.map((index) => cardMarkup(index)).join(''); }
function renderRecent() { const recentGrid = $('#recent-grid'); if (!recentGrid) return; if (!state.recent.length) { recentGrid.innerHTML = '<div class="empty-state">Ouvre ton premier pack pour remplir cet espace.</div>'; return; } recentGrid.innerHTML = state.recent.slice(0, 5).map((index, order) => { const card = cardData(index); return `<article class="recent-card" style="--card-color:${card.color}"><span class="card-number">${String(index + 1).padStart(2, '0')}</span>${order < 2 ? '<span class="new-badge">NOUVEAU</span>' : ''}<span class="mini-art">${card.glyph}</span><span class="card-name">${card.name}</span></article>`; }).join(''); }
